...
**状态:** ✅ 已收尾（2026-07-12 F4 批）
**优先级:** ~~低-中~~（已闭环）
**涉及文件:** `src-engine/vector/chunker.ts`、`src-engine/services/rag_manager.ts`、`src-engine/vector/engine.ts`、`src-ui/src/api/engine-{state,lore,trash,import,chapters}.ts`

**记账前提被推翻**：原判「改造需重建存量向量库（真金白银 embedding）」不成立——分片里
content/embedding/metadata 全存，**metadata 重扫零 API 花费**（纯本地）。用户裁定按
上线前口径直接做（不设拍板仪式）。

**缺口① 已通**：`scanChunkCharacters` 单源判据（与状态扫描同 scanCharactersInChapter）
+ 别名表穿线 splitChapterIntoChunks→prepareChapterChunks→indexChapter/rebuildForAu 全链，
四个索引供表点（confirm/updateChapterContent/backfill/rebuildIndex）接通。存量库走
`JsonVectorEngine.update_chapter_characters` + `RagManager.rescanChunkCharacters`（AU 写锁
+ epoch + changed=0 短路 + persist 失败驱逐自愈）；触发点 = recalcState / 角色卡增删改
恢复（UI api 层 fire-and-forget，per-AU latest-only 队列防旧快照乱序覆盖）。

**缺口② 已通（且比记账更宽）**：executeImportPlan 此前连 castRegistry 都没供（导入
章节 characters_last_seen 恒空）——现 castRegistry+aliases 齐供；导入目标 AU 角色卡
本就在盘上，「供表时机有序性」担忧不成立。bundle 恢复不做向量索引（恒 STALE→rebuild
已带表），无缺口。

测试 +8（chunker 别名/null 对照/判据单源 3 + rescan 命中/round-trip/零 embed/幂等/
故障自愈/隔离 5）。对抗审：codex needs-fix→2 MED（persist 自愈缺口、乱序覆盖）全采纳
整改；opus safe-with-nits 七面全过（全仓调用点零错位、逗号契约四处一致）。

**遗留观察（不阻断）**：大库（数千 chunk）每次角色卡保存触发全量分片重写（persist
既有全量设计 + changed=0 短路缓解；card 编辑低频，真机若见卡顿再做增量 persist）。

## TD-021: 工具参数 schema 双声明 + frontmatter 写侧双序列化（E2/E5 收敛后残余面）

**状态:** ✅ 已收尾（2026-07-12 F2 批）
**优先级:** ~~低~~（已闭环）
**涉及文件:** `src-engine/domain/frontmatter.ts`（写侧单源）、`domain/__tests__/settings_tools_golden.test.ts`（金标）

**frontmatter 写侧**：引擎 `domain/frontmatter.ts` 新增 `dumpFrontmatterKey`（js-yaml
真 YAML 序列化，与读侧 safeMatter 同库往返），settings-chat 块式
（buildManagedFrontmatterLines）与 lore-utils 流式（setAliasesInContent）两份手写
JSON.stringify 技法全部收编；`preserveManagedFrontmatter` 提取改走 safeMatter 真 YAML
解析——**顺手修掉一个实锤漂移 bug**：流式 `aliases: [a, b]`（别名编辑器旧写法）在旧
手写行级解析下被静默丢弃（modify 路径丢别名）。round-trip 测试三层锁行为：引擎
frontmatter_dump 5 例（对抗值/块列表行手术契约/lineWidth）+ UI frontmatter-utils 7 例
（含流式丢失判别回归）+ lore-utils 块式覆写回归。

**工具 schema 双声明**：金标已捕获——`settings_tools_golden.test.ts` 逐字节锁定
getToolsForMode 三模式的 LLM 可见 schema 文本（fixtures/settings_tools_golden.json，
更新走 `UPDATE_GOLDEN=1` 显式再生成）+ 9 个 simple 工具的 properties/required 字段级
双声明相等断言（B5 名集合锁的字段级下探）。**zod 派生评估结论 = 漂移，不强推**：
...
**状态:** ✅ 主体清偿（2026-07-13）—— C1 helper 扩展 `4eda9d4` + C2-C3 12 文件迁移 `4ae7214`（4 子 agent 并行，净删 ~289 行重复，全套 1579 byte-identical）。剩少数**特殊行为** inline mock 有意保留（helper 无法表达：throw-then-succeed / yield-then-throw mid-stream / timing delay / conditional-throw / async 改章节 CAS）——非债，是必要的定制 mock。
**优先级:** ~~低~~（主体已清）
...
**状态:** ✅ 完全闭环（2026-07-13）—— (A) 持久化键：simple_chat `c8126f9`（13 消息+4 undo 字段）+ settings `e3f040d`（5 自定义服务商字段），均 tolerant-read 迁移层（读 snake??camel、写只出 snake、一次 save 自愈）+ round-trip 自愈测试 + opus 对抗审数据安全 SAFE；(B) 接口方法名 `a6fb79b`（8 接口 24 方法名，89 文件纯 rename、零数据风险、op_type 值等字符串完好）。函数/变量级早已闭环（E9+G1）。**snake/camel 命名混用债彻底清完。**
**优先级:** ~~中~~（已闭环）
