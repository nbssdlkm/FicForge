...
**已知局限（多代理审阅 2026-06-23 发现，留作后续）**：`isLlmOverride` 由 `hydrateAuSettingsForm` 从 llm 各字段**真值推断**，无独立持久化开关位。对**只覆盖 key**（model/api_base 沿用全局）的 AU，删除→恢复把唯一非空字段也清空后推断退回 `false`，覆盖区整体折叠 → 提示不出现。这暴露一个更深的**既存** bug：只覆盖 key 的 AU 一经删除恢复，**覆盖被静默丢失**（退回继承全局），不止是少个提示。根治需给 `Project` 增加持久化的 `llm_override_enabled` 标志（写盘 → dictToProject 带占位符感知的回退推断 → hydrate 直接读），并**决定运行时取舍**：恢复后无 key 的覆盖应「报错提示重填」还是「静默继承全局」（`toProjectLlmQueryInfo.has_override` 也走推断，会被一并影响）。涉及产品决策，单列后续任务。当前提示对「整段覆盖（model/api_base 非空）+ key 被清」这一更常见场景工作正常。
