...
> 50 个端点，基于 API-ALIGNMENT 审计（2026-03-30）。
...
| POST | / | name, data_dir (b) | {name, dir_name, path} |
| GET | /{name}/aus | {name} (p), data_dir (q) | [aus] |
| POST | /{name}/aus | {name} (p), name, fandom_path (b) | {name, path} |
...
| PUT | /{name}/rename | {name} (p), new_name (b) | {status, old_name, new_name, new_dir} |
| PUT | /{name}/aus/{au}/rename | {name}, {au} (p), new_name (b) | {status, old_name, new_name, new_dir} |
...
| POST | /confirm | au_path, chapter_num, draft_id (b), content (b, optional) | {chapter_id, chapter_num, current_chapter} |
...
| Method | Path | 参数 | 返回 |
|--------|------|------|------|
| GET | / | au_path (q) | [{id, content_raw, content_clean, status, type, ...}] |
| POST | / | au_path, chapter_num, fact_data (b) | {fact_id} |
| PUT | /{fact_id} | {fact_id} (p), au_path, updated_fields (b) | {fact_id, revision} |
| PATCH | /{fact_id}/status | {fact_id} (p), au_path, new_status (b) | {fact_id, status} |
| POST | /extract | au_path, chapter_num, session_llm (b) | {facts: [...]} |
...
| Method | Path | 参数 | 返回 |
|--------|------|------|------|
| POST | /stream | au_path, chapter_num, user_input (b) | SSE: context_summary → token* → done |

**SSE 事件顺序**：
1. `event: context_summary` — ContextSummary JSON（可能缺失，前端需容错）
2. `event: token` — `{text: "..."}` 逐 token 推送
3. `event: done` — `{draft_label: "A"}` 生成完成
4. `event: error` — `{error_code, message, actions}` 生成失败
...
| POST | /test-connection | mode, model, api_base, api_key (b) | {success, model, message, error_code} |
...
| POST | /recalc | au_path (b) | {characters_last_seen, last_scene_ending, chapters_scanned} |
...
| POST | /import/confirm | au_path, chapters (b) | {total_chapters, characters_found, state_initialized} |
| GET | /export | au_path, format (q) | File download |
...
**purge**：`max_age_days=0` → 强制清空全部；不传 → 只清已过期（默认 30 天）。
