 ...
 *
 * 为什么放 domain：写路径（services/facts_lifecycle 的 add_fact/edit_fact、
 ...
 * 与 edit_fact 既有的枚举校验语义一致）；`{ ok: true, value }` = 已消毒可落库。
 */
...
function clean_name_list(raw: unknown[], aliases: Record<string, string[]> | null): string[] {
  const names = raw
    .filter((x): x is string => typeof x === "string")
    .map((s) => s.trim())
    .filter((s) => s !== "");
  return [...new Set(normalize_characters(names, aliases))];
}
 ...
 * - 其它类型（数字/对象/布尔）→ 形状非法。
 */
export function sanitize_known_to(
  raw: unknown,
  ...
  if (typeof raw === "string") {
    ...
    return { ok: true, value: clean_name_list([t], aliases) };
  }
  if (Array.isArray(raw)) {
    const names = clean_name_list(raw, aliases);
 ...
 * - 其它类型 → 形状非法。
 */
export function sanitize_hidden_from(
  raw: unknown,
  aliases: Record<string, string[]> | null = null,
): SanitizeResult<string[]> {
  if (raw === null || raw === undefined) return { ok: true, value: [] };
  if (Array.isArray(raw)) return { ok: true, value: clean_name_list(raw, aliases) };
  return { ok: false };
}
 ...
 * 消费点：add_fact / rawToExtracted / edit_fact（知情字段被编辑时）。
 */
export function reconcile_knowledge(
 ...
 * 非对象/数组 → 形状非法。
 */
export function sanitize_confidence(raw: unknown): SanitizeResult<FactFieldConfidence | undefined> {
