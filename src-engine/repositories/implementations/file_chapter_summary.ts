...
import { atomicWrite, joinPath, nowUtc } from "../../utils/file_utils.js";

/** ch{NNNN}.summary.jsonl 路径。NNNN 为 4 位零填充章节号。 */
export function summaryPath(au_id: string, chapterNum: number): string {
  const padded = String(chapterNum).padStart(4, "0");
  return joinPath(au_id, "chapters", "main", `ch${padded}.summary.jsonl`);
}

export class FileChapterSummaryRepository implements ChapterSummaryRepository {
  constructor(private adapter: PlatformAdapter) {}

  async get(au_id: string, chapterNum: number): Promise<ChapterSummary | null> {
    const path = summaryPath(au_id, chapterNum);
  ...
  async save(au_id: string, chapterNum: number, summary: ChapterSummary): Promise<void> {
    const path = summaryPath(au_id, chapterNum);
  ...
  async remove(au_id: string, chapterNum: number): Promise<void> {
    const path = summaryPath(au_id, chapterNum);
    if (await this.adapter.exists(path)) await this.adapter.deleteFile(path);
  }

  /** 读取现有摘要（或空初始），合并写入 micro 键。幂等（后者覆盖）。 */
  async update_micro(au_id: string, chapterNum: number, text: string, hash: string): Promise<void> {
    const existing = (await this.get(au_id, chapterNum)) ?? createChapterSummary({});
    ...
    await this.save(au_id, chapterNum, updated);
  }
   ...
   */
  async promote_to_v2(au_id: string, chapterNum: number, v2Text: string, hash: string): Promise<void> {
    const existing = (await this.get(au_id, chapterNum)) ?? createChapterSummary({});
