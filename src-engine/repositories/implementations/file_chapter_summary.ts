...
export function summaryPath(au_id: string, chapter_num: number): string {
  const padded = String(chapter_num).padStart(4, "0");
...
export class FileChapterSummaryRepository implements ChapterSummaryRepository {
  ...
  async get(au_id: string, chapter_num: number): Promise<ChapterSummary | null> {
    const path = summaryPath(au_id, chapter_num);
  ...
  async save(au_id: string, chapter_num: number, summary: ChapterSummary): Promise<void> {
    const path = summaryPath(au_id, chapter_num);
  ...
  async remove(au_id: string, chapter_num: number): Promise<void> {
    const path = summaryPath(au_id, chapter_num);
  ...
  async updateMicro(au_id: string, chapter_num: number, text: string, hash: string): Promise<void> {
    ...
    await this.save(au_id, chapter_num, updated);
  }
