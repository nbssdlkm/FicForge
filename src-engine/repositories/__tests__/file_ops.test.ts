...
describe("FileOpsRepository", () => {
  ...
  it("returns empty list when file missing", async () => {
    const entries = await repo.listAll("au1");
  ...
  it("listByChapter filters correctly", async () => {
    await repo.append(
      "au1",
    ...
    const ch1 = await repo.listByChapter("au1", 1);
    expect(ch1).toHaveLength(2);
  });

  it("getConfirmForChapter returns latest", async () => {
    await repo.append(
      "au1",
    ...
    const latest = await repo.getConfirmForChapter("au1", 1);
    expect(latest).not.toBeNull();
    expect(latest!.op_id).toBe("op2");
  });

  it("getConfirmForChapter returns null when none", async () => {
    const result = await repo.getConfirmForChapter("au1", 99);
    expect(result).toBeNull();
  });

  it("getAddFactsForChapter", async () => {
    await repo.append(
      "au1",
    ...
    const facts = await repo.getAddFactsForChapter("au1", 1);
    expect(facts).toHaveLength(1);
  });

  it("getLatestByType returns last entry", async () => {
    await repo.append(
      "au1",
    ...
    const latest = await repo.getLatestByType("au1", "rebuild_index");
    expect(latest!.op_id).toBe("op2");
  });
