 ...
 * - listFandoms 在空 dataDir 下正常工作，不抛 "data_dir must not be empty"
...
describe("FileFandomRepository.listFandoms (空 dataDir)", () => {
  it("无 fandoms 目录 → 返回 []，不抛错（回归保护）", async () => {
    const adapter = new MockAdapter();
    const repo = new FileFandomRepository(adapter, "");
    await expect(repo.listFandoms()).resolves.toEqual([]);
  });
  ...
  it("列出所有含 fandom.yaml 的子目录", async () => {
    ...
    const repo = new FileFandomRepository(adapter, "");
    expect(await repo.listFandoms()).toEqual(["HP", "SNK"]);
  });
  ...
  it("过滤掉 fandom.yaml 已被 trash 的目录（deleteFandom 只 trash fandom.yaml）", async () => {
    ...
    const repo = new FileFandomRepository(adapter, "");
    expect(await repo.listFandoms()).toEqual(["HP"]);
  });
});

describe("FileFandomRepository.listFandoms (非空 dataDir)", () => {
...
describe("FileFandomRepository.listAus", () => {
  ...
  it("没有 aus 目录时返回 []", async () => {
    ...
    adapter.seed("fandoms/HP/fandom.yaml", "name: HP\n");
    const repo = new FileFandomRepository(adapter, "");
    expect(await repo.listAus("fandoms/HP")).toEqual([]);
  });

  it("拒绝空 fandom_path", async () => {
    const adapter = new MockAdapter();
    const repo = new FileFandomRepository(adapter, "");
    await expect(repo.listAus("")).rejects.toThrow(/must not be empty/);
  });
});
