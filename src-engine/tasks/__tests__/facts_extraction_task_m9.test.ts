...
import { createFactsExtractionTask, type FactsExtractionParams } from "../impl/facts-extraction-task.js";
...
function toolIter(calls: { name: string; args: object }[]): LLMChunk[] {
  ...
  calls.forEach((c, i) => {
    chunks.push({ delta: "", tool_call_deltas: [{ index: i, id: `c_${i}_${c.name}`, type: "function", function: { name: c.name, arguments: "" } }], is_final: false, input_tokens: i === 0 ? 5 : null, output_tokens: null, finish_reason: null });
...
function scriptedProvider(iters: LLMChunk[][]): LLMProvider {
  let i = 0;
  return {
    async generate(): Promise<LLMResponse> { return { content: "", model: "m", input_tokens: 0, output_tokens: 0, finish_reason: "stop" }; },
    async *generateStream(): AsyncIterable<LLMChunk> {
      const it = i < iters.length ? iters[i] : [{ delta: "done", is_final: true, input_tokens: 1, output_tokens: 1, finish_reason: "stop" } as LLMChunk];
      i++;
...
describe("批量提取任务 — M9 接线", () => {
  const params = (over: Partial<FactsExtractionParams> = {}): FactsExtractionParams =>
    ({ auPath: "au", fromChapter: 1, toChapter: 2, batchSize: 2, language: "zh", ...over });
  ...
  it("reactExtractionEnabled：逐章走 ReAct，每章事实带内联 thread_ids", async () => {
    ...
    const provider = scriptedProvider([
      toolIter([{ name: "propose_facts", args: { facts: [{ content_clean: "第一章的事实内容", characters: ["林晚月"], thread_ids: ["t1"] }] } }]),
      toolIter([{ name: "finalize_extraction", args: {} }]),
      toolIter([{ name: "propose_facts", args: { facts: [{ content_clean: "第二章的事实内容", characters: ["林晚月"], thread_ids: ["t1"] }] } }]),
      toolIter([{ name: "finalize_extraction", args: {} }]),
    ]);
