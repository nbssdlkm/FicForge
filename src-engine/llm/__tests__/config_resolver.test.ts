...
import { createProvider, resolveLlmConfig, resolveLlmParams } from "../config_resolver.js";
...
describe("resolveLlmParams", () => {
  it("session_params takes priority", () => {
    const result = resolveLlmParams("gpt-4o", { temperature: 0.5, top_p: 0.8 }, {}, {});
    expect(result.temperature).toBe(0.5);
    expect(result.top_p).toBe(0.8);
  });

  it("falls to project override", () => {
    const result = resolveLlmParams(
      "gpt-4o",
      null,
  ...
  it("falls to defaults", () => {
    const result = resolveLlmParams("unknown", null, {}, {});
...
// ---------------------------------------------------------------------------
// createProvider —— P1-5a 真正支持 ollama
// ---------------------------------------------------------------------------
...
describe("createProvider", () => {
  ...
  it("mode=api 空 api_base 抛错（堵死相对 URL / 协议相对 chat_path 外泄链——盲审 R3 HIGH-2）", () => {
    expect(() => createProvider({ mode: "api", model: "m", api_base: "", api_key: "sk-x" })).toThrow(/api_base/);
    expect(() =>
      createProvider({ mode: "api", model: "m", api_base: "   ", api_key: "sk-x", chat_path: "//attacker/v1" }),
    ).toThrow(/api_base/);
  });
  ...
  it("mode=ollama 缺 ollama_model 抛错（引擎级护栏）", () => {
    expect(() => createProvider({ mode: "ollama", model: "", api_base: "", api_key: "" })).toThrow(/ollama_model/i);
  });

  it("mode=local 抛错（sidecar 退役后本版本不支持）", () => {
    expect(() => createProvider({ mode: "local", model: "", api_base: "", api_key: "" })).toThrow(
      /不支持.*local|local.*不支持|本地模型|not.*implemented/i,
    );
  });

  it("未知 mode 抛错", () => {
    expect(() => createProvider({ mode: "anthropic-native", model: "m", api_base: "", api_key: "" })).toThrow(/mode/i);
  });
});

describe("resolveLlmConfig — context_window 同层同源（审计 H4）", () => {
  ...
  it("context_window = 0 是「自动推断」哨兵：视同未指定", () => {
    const r = resolveLlmConfig(null, { llm: { mode: "api", model: "m-proj", context_window: 0 } }, { default_llm: {} });
    expect(r.context_window).toBeUndefined();
  });
});

describe("resolveLlmConfig — chat_path 同层同源（仿 H4 context_window）", () => {
...
describe("resolveLlmConfig — session 继承需 api_base 同源（审计 3-A 终审复现）", () => {
