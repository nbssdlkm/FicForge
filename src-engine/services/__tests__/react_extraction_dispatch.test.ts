...
import { buildFactEnrichmentSuffix, buildFactKnowledgeClause } from "../context_assembler.js";
...
import type { LLMChunk } from "../../llm/provider.js";
...
function toolIter(calls: { name: string; args: object }[]): LLMChunk[] {
  ...
  calls.forEach((c, i) => {
    chunks.push({
      delta: "",
      tool_call_deltas: [
        { index: i, id: `call_${i}_${c.name}`, type: "function", function: { name: c.name, arguments: "" } },
      ],
      is_final: false,
      input_tokens: i === 0 ? 10 : null,
      output_tokens: null,
      finish_reason: null,
    });
    chunks.push({
...
const SEED_FACT = createFact({
  id: "f_seed_3",
  content_raw: "r",
  content_clean: "林晚月炼气失败灵力枯竭",
  characters: ["林晚月"],
  chapter: 3,
});
      ...
      toolIter([
        {
          ...
          args: { facts: [{ content_clean: "林晚月灵力虚弱", characters: ["林晚月"], evidence: "灵力虚弱" }] },
        },
      ]),
      ...
      toolIter([
        {
          ...
          args: { fact_index: 0, caused_by_fact_ids: ["f_seed_3"], thread_ids: ["t_seed"] },
        },
      ]),
  ...
  it("未触发上限时 cappedCount = 0", async () => {
    const provider = createScriptedStreamProvider([
      toolIter([
        {
          name: REACT_TOOL_PROPOSE,
          args: { facts: [{ content_clean: "只有一条事实内容", characters: [], evidence: "结盟" }] },
        },
      ]),
      toolIter([{ name: REACT_TOOL_FINALIZE, args: {} }]),
    ]);
  ...
  it("annotate 编造的 fact_id / thread_id 被丢弃（只保真实存在的）", async () => {
    ...
    const provider = createScriptedStreamProvider([
      toolIter([
        {
          name: REACT_TOOL_PROPOSE,
          args: { facts: [{ content_clean: "某事件发生了", characters: [], evidence: "结盟" }] },
        },
      ]),
      toolIter([
        {
          name: REACT_TOOL_ANNOTATE,
          args: { fact_index: 0, caused_by_fact_ids: ["f_seed_3", "f_HALLUCINATED"], thread_ids: ["t_seed", "t_FAKE"] },
        },
      ]),
      toolIter([{ name: REACT_TOOL_FINALIZE, args: {} }]),
    ]);
  ...
  it("ungrounded 因果边仍挂上，但标 _confidence.caused_by=low（flag 不 gate；人审兜底）", async () => {
    ...
    const provider = createScriptedStreamProvider([
      ...
      toolIter([
        {
          name: REACT_TOOL_PROPOSE,
          args: { facts: [{ content_clean: "这是一条无依据事实", characters: [], evidence: "皇宫夜宴" }] },
        },
      ]),
      ...
      toolIter([
        {
          ...
          args: {
            facts: [
              {
                content_clean: "某个有效的事实内容",
                characters: [],
                evidence: "结盟",
                thread_ids: ["t_seed", "t_FAKE"],
                caused_by_fact_ids: ["f_seed_3", "f_FAKE"],
              },
            ],
          },
        },
  ...
  it("早停（Finding 3）：已产出事实后再次 propose → 终止 status ok，不空转到 maxIter", async () => {
    ...
    const provider = createScriptedStreamProvider([
      toolIter([
        {
          name: REACT_TOOL_PROPOSE,
          args: { facts: [{ content_clean: "林晚月首次提议的关键事实", characters: ["林晚月"], evidence: "结盟" }] },
        },
      ]),
      toolIter([
        {
          name: REACT_TOOL_PROPOSE,
          args: { facts: [{ content_clean: "林晚月第二次重复提议", characters: ["林晚月"] }] },
        },
      ]),
    ]);
    const res = await reactExtractFromChapter(CHAPTER, 5, [], { characters: ["林晚月"] }, null, provider, {
      ...
      toolIter([
        {
          ...
          args: {
            facts: [
              {
                content_clean: "林晚月与人结盟对抗强敌",
                characters: ["林晚月"],
                fact_type: "NOT_A_REAL_ENUM",
                caused_by: ["f_seed_3", "f_HALLUCINATED"],
              },
            ],
          },
        },
...
describe("reactExtractFromChapter — 终止 / 降级语义", () => {
  ...
  it("空章节 → 空结果 status ok（不跑 LLM）", async () => {
    const provider = createScriptedStreamProvider([textIter("never called")]);
    const res = await reactExtractFromChapter("   ", 5, [], { characters: [] }, null, provider, {
      _telemetry_override: silentTelemetry,
    });
...
describe("reactExtractFromChapter — round-trip 闭环（最高优先级）", () => {
  it("提取产出的 caused_by + thread_ids 经 addFact → list_all + ops rebuild 读回一致", async () => {
    ...
    // 模拟 UI 确认步：把提取候选（含 caused_by/thread_ids）落库（addFact）。新建独立 AU 的 repo。
...
describe("reactExtractFromChapter — codex 二审修复回归", () => {
  ...
  it("annotate 分两次补 caused_by → 合并不覆盖（union）", async () => {
    ...
    await factRepo.append(
      "au",
      createFact({ id: "f_seed_2", content_raw: "r", content_clean: "另一条更早的事实", characters: [], chapter: 2 }),
    );
    const provider = createScriptedStreamProvider([
      toolIter([
        {
          name: REACT_TOOL_PROPOSE,
          args: { facts: [{ content_clean: "承前的事实内容", characters: [], evidence: "灵力虚弱" }] },
        },
      ]),
      toolIter([{ name: REACT_TOOL_ANNOTATE, args: { fact_index: 0, caused_by_fact_ids: ["f_seed_3"] } }]),
  ...
  it("一章 propose 超量 → 按 REACT_MAX_FACTS_PER_CHAPTER 软上限截断（防过度提取）", async () => {
    const facts = Array.from({ length: REACT_MAX_FACTS_PER_CHAPTER + 4 }, (_, i) => ({
      content_clean: `第${i}条独立有效的事实内容`,
      characters: [],
    }));
    const provider = createScriptedStreamProvider([
      toolIter([{ name: REACT_TOOL_PROPOSE, args: { facts } }]),
...
describe("reactExtractFromChapter — H10 富化字段置信度合成（回归锚：删合成逻辑必挂）", () => {
  it("propose 带 location+known_to+time_kind → _confidence 各字段=medium，且 P3 注入门控产出非空后缀", async () => {
    ...
    expect(buildFactKnowledgeClause(res.facts[0] as unknown as Fact, "zh")).toBe("（仅林晚月知道）");
  });
  ...
  it("富化字段全空 → 不凭空造 _confidence（保持 undefined，与门控 `!c` 短路语义兼容）", async () => {
    const provider = createScriptedStreamProvider([
      toolIter([
        { name: REACT_TOOL_PROPOSE, args: { facts: [{ content_clean: "无任何富化字段的事实", characters: [] }] } },
      ]),
      toolIter([{ name: REACT_TOOL_FINALIZE, args: {} }]),
    ]);
    const res = await reactExtractFromChapter(CHAPTER, 5, [], { characters: [] }, null, provider, {
  ...
  it("端到端：ReAct 提取 → addFact 落库 → jsonl 读回 + ops rebuild 的 fact 过门控均产非空后缀", async () => {
    const provider = createScriptedStreamProvider([
      toolIter([
        {
          name: REACT_TOOL_PROPOSE,
    ...
    const clause = buildFactKnowledgeClause(persisted, "zh");
      ...
      toolIter([
        {
          ...
          args: {
            facts: [
              { content_clean: "abc", characters: [] },
              { content_clean: "够长的有效事实内容", characters: [] },
            ],
          },
        },
      ]),
