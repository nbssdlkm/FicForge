...
/**
 * TDD tests for runArchivalSweep (Phase B cold-tier archival).
 */
...
describe("run_archival_sweep", () => {
  ...
  it("archives fact that satisfies distance + low weight + active status", async () => {
    ...
    const archived = await runArchivalSweep("au1", 11, factRepo, opsRepo);
  ...
  it("accepts custom threshold", async () => {
    ...
    const archived = await runArchivalSweep("au1", 7, factRepo, opsRepo, 5);
...
describe("isArchivalCandidate (predicate boundaries)", () => {
  ...
  it("archived === undefined (legacy fact) → treated as not-archived → candidate", () => {
    expect(isArchivalCandidate({ ...base({ chapter: 1 }), archived: undefined as unknown as boolean }, 11, 10)).toBe(
      true,
    );
  });
...
// Q4 用户确认流的两个原语：find（只读预览） + archiveFacts（归档确认子集）。
describe("findArchivalCandidates (read-only preview)", () => {
  let adapter: MockAdapter;
  let factRepo: FileFactRepository;
  let opsRepo: FileOpsRepository;
  beforeEach(() => {
  ...
  it("returns cold candidates WITHOUT mutating them (no archive op written)", async () => {
    ...
    const candidates = await findArchivalCandidates("au1", 11, factRepo);
...
describe("archiveFacts (archive confirmed subset)", () => {
  let adapter: MockAdapter;
  let factRepo: FileFactRepository;
  let opsRepo: FileOpsRepository;
  beforeEach(() => {
  ...
  it("archives only the given ids (user's confirmed subset), leaves the rest untouched", async () => {
    ...
    const archived = await archiveFacts("au1", [a.id], factRepo, opsRepo);
  ...
  it("is idempotent: skips already-archived and missing ids", async () => {
    ...
    await archiveFacts("au1", [a.id], factRepo, opsRepo);
    const archived = await archiveFacts("au1", [a.id, "f_missing"], factRepo, opsRepo);
