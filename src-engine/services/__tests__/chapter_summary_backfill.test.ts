...
import { findChaptersMissingSummary } from "../chapter_summary.js";

// 注：原 backfill_chapter_summaries 用例随该函数退役删除（被「补全旧章记忆」统一 pass 取代，
// 见 services/__tests__/backfill_memory.test.ts）。findChaptersMissingSummary 作为「缺摘要」
// 单一真相源判据保留，仍被 scanChapterMemory / backfillChapterMemory 复用，故保留其回归。

describe("findChaptersMissingSummary", () => {
  it("returns only chapters lacking a non-empty standard summary", async () => {
    ...
    const missing = await findChaptersMissingSummary("/au", [1, 2, 3, 4, 5], summaryRepo);
