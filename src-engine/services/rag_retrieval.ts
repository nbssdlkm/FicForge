...
export function buildRagQuery(focus_texts: string[], last_scene_ending: string, user_input: string): string {
  const parts: string[] = [];
...
export function buildActiveChars(
 ...
 * 衰减后必须重排序：否则保持 cosine 序，预算裁剪 reduceTopK 可能留旧弃新（codex 对抗审）。
...
export async function retrieveRag(
  ...
  for (const collName of collections) {
    ...
    const chunks = await searchCollection(vector_repo, au_id, queryEmbedding, collName, topK, char_filter);
    for (const c of chunks) {
  ...
  const chChunks = await searchCollection(vector_repo, au_id, queryEmbedding, "chapters", CHAPTERS_TOP_K, char_filter);
  ...
  // retrieveRag 收到的 current_chapter 是"待写章"，P2 注入的是 current-1，故排除 chNum >= current-1。
  // 摘要是整章级、非角色作用域，且其 chunk 无 characters metadata：若传 char_filter，
  // 主查询必返 0 再触发兜底全局查询 = 每次双查（codex workflow 审）。直接传 null 走单查询。
  const sumChunks = await searchCollection(vector_repo, au_id, queryEmbedding, "summaries", SUMMARIES_TOP_K, null);
  allChunks.push(
  ...
  let tokens = countTokens(text, llm_config as { mode?: string }).count;
  ...
  if (tokens > budget_remaining && budget_remaining > 0) {
    for (const reducedK of [2, 1]) {
      ...
      tokens = countTokens(text, llm_config as { mode?: string }).count;
    ...
    for (const prioColl of priority) {
      for (const c of deduped) {
        if (c._collection === prioColl) {
          const cTokens = countTokens(c.content, llm_config as { mode?: string }).count;
...
function reduceTopK(chunks: ChunkWithCollection[], maxPerCollection: number): ChunkWithCollection[] {
...
// ---------------------------------------------------------------------------
//
// 把「buildActiveChars → focusTexts → buildRagQuery → retrieveRag」这套编排
// 从 generation.ts 抽出,已 export 并接入 services barrel。当前 caller = generateChapter
// (写文路径);融合后续步骤(plan §1.1-1.3)对话路径(simple_chat_dispatch)复用本函数 ——
// 目的是杜绝两套手工维护的 RAG 编排漂移(单一真相源)。
//
// 行为与原 generation.ts:218-244 内联块逐字节等价:空 query → 不检索;retrieveRag
...
export interface RetrieveRagForContextArgs {
  ...
  language?: string;
  /**
   * H4：实际生效 LLM 视图（resolveLlmConfig 结果，{model, context_window} 足够）。
   * ragBudget（≈ctx/4）按它算窗口；可选 + 缺省回退 project.llm —— 旧调用方不传时
   * 行为不变（向后兼容硬约束）。
   */
  effective_llm?: { model?: string; context_window?: number } | null;
  /**
   * E8：角色别名表（主名 → 别名列表）。透传给 buildActiveChars —— 正文/用户输入只出现别名
...
export async function retrieveRagForContext(
  ...
  try {
    ...
    // E8：别名表透传 buildActiveChars —— 正文只出现别名时主名也进 char_filter（缺省 null = 现状）。
    const activeChars = buildActiveChars(state, user_input, project, facts, castReg, character_aliases);
    // 审计⑥：已归档冷 fact 的 content_clean 不进 RAG 检索 query，避免把召回拉向本应冷藏的旧线
    // （与 buildFactsLayer / FOCUS_GOAL 同用 isColdFact 单一真相源）。
    ...
    const query = buildRagQuery(focusTexts, lastEnding, user_input);
    if (!query) return { ragText: null, chunks: [] };

    // 用 getContextWindow(支持 context_window=0 自动按 model 推断),与 context_assembler 同源;
    // 原内联块写死 `|| 128000`,在 context_window=0 + 大上下文模型时会把预算算小(审计 R2 修)。
    // H4：给了 effective 视图则按实际生效模型算窗口（缺省回退 project.llm，向后兼容）。
    const ragBudget = Math.max(0, Math.trunc(getContextWindow(effective_llm ? { llm: effective_llm } : project) / 4));
    const [ragResult, , chunks] = await retrieveRag(
      vector_repo,
      embedding_provider,
      au_id,
      query,
