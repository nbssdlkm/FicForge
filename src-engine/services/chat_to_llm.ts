 ...
 *
 * 原实现在 src-ui/src/ui/simple/chat-to-llm.ts；「聊天历史 → LLM messages」的
 * 序列化规则（[已执行]/[待确认]/草稿状态 marker 等影响 prompt 的业务判断）属于
 * 生成流水线，归引擎层（盲审长期债④第二步下沉至此，输出逐字节不变，golden 基线
 * 见 src-ui `__tests__/chat-to-llm.golden.test.ts`）。UI 端保留薄 re-export。
 *
