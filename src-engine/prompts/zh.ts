...
const zh: PromptModule = {
  ...
  ATTENTION_BODY: "以下悬念极易被顺带提及，请特别克制，本章保持悬而未决：\n" + "{caution_lines}",
  ...
  INFO_ASYMMETRY_RULES:
    ...
    "不知情的角色绝不能提及、暗示或依据该事实行动，其言行必须符合认知盲区；可利用这种不对称制造张力。\n" +
    "「仅读者知」表示所有角色均不知情，任何角色都不得说破；除非剧情安排其得知或指令明确要求揭晓。）",
  ...
  WORD_COUNT_REMINDER:
    "【重要提醒】本章必须控制在 {chapter_length} 字以内。接近该字数时立即收束场景，写出结尾。宁可少写也不要超字数。",
  ...
  SUMMARY_STANDARD_USER: "为第 {chapter_num} 章写 180-250 字叙事摘要：\n\n{chapter_text}",
  ...
  SUMMARY_MICRO_USER: "为第 {chapter_num} 章写 30-50 字章节名片：\n\n{chapter_text}",

  THREAD_STATE_SYSTEM:
    "你是剧情线进展维护助手。根据一条剧情线的名称、简介与已发生事件，用**一句话**（不超过 40 字）" +
    "概括这条线的「当前进展」——即它现在推进到了哪一步。要求：①第三人称客观；②只输出这一句话，" +
    "不要解释、不要引号、不要分点、不要前言；③不编造事件里没有的内容。",
  THREAD_STATE_USER:
    "剧情线：{title}\n简介：{description}\n\n已发生事件（按时间先后）：\n{facts}\n\n请用一句话概括「当前进展」：",
  ...
  CHAPTER_TITLE_PROMPT:
    "为以下小说章节起一个简短的中文标题（不超过10个字），只返回标题文字，不要任何解释或标点符号：\n\n{content}",
