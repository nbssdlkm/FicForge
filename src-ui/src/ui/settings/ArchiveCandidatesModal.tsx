 ...
 */
export function ArchiveCandidatesModal({ auPath, isOpen, onClose }: { auPath: string; isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <Modal isOpen={isOpen} onClose={requestClose} title={t('archive.title')}>
      <div className="space-y-4">
        {phase === 'scanning' && (
          <div className="flex items-center gap-3 py-4 text-sm text-text/70"><Spinner size="md" /> {t('archive.scanning')}</div>
        )}

        {(phase === 'confirm' || phase === 'archiving') && (
          candidates.length === 0 ? (
            <EmptyState compact icon={<Archive size={28} />} title={t('archive.noneTitle')} description={t('archive.none')} />
          ) : (
            <>
              ...
              <div className="max-h-[44vh] space-y-2 overflow-y-auto pr-1">
                {candidates.map((f) => {
                  ...
                  return (
                    <label key={f.id} className={`flex cursor-pointer gap-3 rounded-lg border p-3 dark:border-white/10 ${checked ? 'border-accent/40 bg-accent/5' : 'border-black/10 bg-surface/40'}`}>
                      <input type="checkbox" className="mt-1 accent-accent" checked={checked} disabled={busy} onChange={() => toggle(f.id)} />
                      <div className="min-w-0 flex-1 space-y-1">
                        <p className="text-sm text-text/90">{f.content_clean}</p>
                        <div className="flex flex-wrap items-center gap-2">
                          <Tag tone="default">{getEnumLabel('narrative_weight', f.narrative_weight, f.narrative_weight)}</Tag>
                          <span className="text-xs text-text/50">{t('facts.extractSourceChapter', { chapter: f.chapter })}</span>
                        </div>
                      </div>
                    </label>
                  );
        ...
        {phase === 'done' && (
          <p className="text-sm text-text/80">{t('archive.done', { count: archivedCount })}</p>
        )}
        ...
        <div className="flex justify-end gap-2 border-t border-black/10 pt-4 dark:border-white/10">
          {phase === 'confirm' && candidates.length > 0 ? (
            ...
            <Button tone="accent" fill="solid" disabled><Spinner size="md" /></Button>
          ) : phase !== 'scanning' ? (
