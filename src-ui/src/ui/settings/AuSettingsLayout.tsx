...
export const AuSettingsLayout = ({ auPath }: { auPath: string }) => {
  ...
  return (
    <>
      <main className="flex-1 overflow-y-auto w-full">
        <div className="mx-auto max-w-4xl space-y-10 px-4 py-4 md:p-8 lg:p-12">
          <header className="flex flex-col gap-4 border-b border-black/10 pb-6 dark:border-white/10 md:flex-row md:items-center md:justify-between">
            <div className="flex items-center gap-3">
              <h1 className="flex flex-wrap items-center gap-2 font-serif text-xl font-bold md:text-2xl">
                ...
                <span className="text-lg font-normal opacity-50 ml-2">
                  {t("settings.story.scopeLabel", { name: auName })}
                </span>
              </h1>
            </div>
            <Button
              tone="accent"
              fill="solid"
              className="w-full gap-2 shadow-md md:w-24"
              onClick={save}
              disabled={saving}
            >
          ...
          <section className="space-y-4">
            <h2 className="text-lg font-sans font-bold text-accent border-l-4 border-accent pl-3">
            ...
            <div className="space-y-4">
              <div className="flex flex-col gap-4 md:flex-row md:items-center md:justify-between">
                ...
                <div className="flex flex-wrap items-center gap-3">
                  <Toggle
                    checked={form.isLlmOverride}
                    onChange={(e) => setIsLlmOverride(e.target.checked)}
                    label={t("settings.story.overrideToggle")}
                  />
                  <Button tone="neutral" fill="plain" size="sm" onClick={modals.openGlobalSettings}>
                    {t("common.actions.viewGlobalSettings")}
                  </Button>
                </div>
              </div>
                      ...
                      <div className="flex flex-col gap-1.5">
                        ...
                        {shouldWarnEmptyAuApiKey(form.isLlmOverride, form.llmMode, form.auApiKey) ? (
                          <p className="text-xs text-amber-600 dark:text-amber-500">
                            {t("settings.story.apiKeyEmptyHint")}
                          </p>
                        ) : (
          ...
          <section className="space-y-4">
            <h2 className="text-lg font-sans font-bold text-info border-l-4 border-info pl-3">
              {t("settings.sections.searchEngine")}
            </h2>
            <div className="space-y-4">
              <div className="flex flex-col gap-2">
                ...
                {!form.isEmbeddingOverride && (
                  <Input
                    ...
                    readOnly
                    className="h-11 bg-background/70 font-mono text-base md:h-10 md:text-sm"
                  />
                )}
                <label className="flex min-h-[44px] items-center gap-2 text-sm cursor-pointer">
                  <input
                    type="checkbox"
                    checked={form.isEmbeddingOverride}
                    onChange={(e) => setIsEmbeddingOverride(e.target.checked)}
                    disabled={saving}
                ...
                {form.isEmbeddingOverride && (
                  <div className="space-y-2 pl-6 border-l-2 border-info/30">
                    ...
                    <Input
                      ...
                      placeholder={t("settings.global.embeddingApiKeyPlaceholder")}
                      disabled={saving}
                      className="h-11 text-base md:h-8 md:text-sm"
                      type="password"
                    />
              ...
              <div className="flex flex-col items-start gap-2 rounded-lg border border-black/10 dark:border-white/10 bg-background/60 px-4 py-3">
                ...
                <Tag tone="info" className="text-xs">
                  {getEnumLabel("index_status", indexStatus, indexStatus)}
                </Tag>
              </div>
            </div>
          </section>
          ...
          <section className="space-y-6">
            <h2 className="text-lg font-sans font-bold text-success border-l-4 border-success pl-3">
              {t("settings.sections.coreIncludes")}
            </h2>
            ...
            <div className="flex gap-3 flex-wrap">
              ...
              <Button
                ...
                fill="plain"
                size="sm"
                className="h-11 border border-dashed border-success/30 text-sm text-success hover:bg-success/5 md:h-8 md:text-xs"
                onClick={modals.openCoreInclude}
              >
          ...
          {project?.cast_registry && (
            <section className="space-y-6">
              <h2 className="text-lg font-sans font-bold text-info border-l-4 border-info pl-3">
                {t("settings.sections.castRegistry")}
              </h2>
                ...
                {(project.cast_registry.characters || []).length === 0 ? (
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.cast_registry.characters.map((c) => (
                      <span
                        key={c}
                        className="inline-flex items-center gap-1 rounded-md bg-black/5 dark:bg-white/5 px-2 py-1 text-xs"
                      >
                        {c}
                        <button
                          type="button"
      ...
      <Modal
        isOpen={modals.isCoreIncludeOpen}
        onClose={modals.closeCoreInclude}
        title={t("settings.createCoreIncludeTitle")}
      >
        <div className="space-y-4">
          {(() => {
          ...
          <div className="flex justify-end">
            <Button tone="neutral" fill="plain" onClick={modals.closeCoreInclude}>
