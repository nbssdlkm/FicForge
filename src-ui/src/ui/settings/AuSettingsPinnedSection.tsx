    ...
    <section className="space-y-6">
      <h2 className="flex flex-col gap-3 border-l-4 border-error pl-3 text-lg font-sans font-bold text-error md:flex-row md:items-center md:justify-between">
        ...
        <Button
          ...
          fill="outline"
          size="sm"
          className="h-11 border-error/30 text-sm font-normal text-error hover:bg-error/10 md:h-8 md:text-xs"
          onClick={addPinnedRule}
        >
        ...
        {pinnedContext.length === 0 ? (
          ...
          pinnedContext.map((pc, idx) => (
            ...
            <div key={idx} className="flex items-start gap-3 rounded-lg border border-error/20 bg-error/5 p-4">
              ...
              <Textarea
                className="min-h-[60px] flex-1 bg-background font-serif text-base md:text-sm"
                value={pc}
                onChange={(e) => updatePinnedRule(idx, e.target.value)}
              />
              <Button
                tone="neutral"
                fill="plain"
                size="sm"
                className="h-11 w-11 p-0 text-error/60 hover:bg-error/10 hover:text-error md:h-auto md:w-auto md:p-2"
                onClick={() => removePinnedRule(idx)}
              >
                <Trash2 size={16} />
              </Button>
            </div>
