...
export const GlobalSettingsModal = ({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) => {
  ...
  const llmConnection = useLlmConnectionTest({
    ...
    getExceptionMessage: (error) =>
      `${t("settings.global.testFailedPrefix")}${error instanceof Error ? error.message || t("error_messages.unknown") : t("error_messages.unknown")}`,
  });
      ...
      {loading ? (
        ...
        <div className="mt-4 space-y-6">
          ...
          <div className="space-y-4">
            <div className="flex items-center justify-between">
              ...
              <Button
                ...
                onClick={modals.openApiHelp}
              >
                <HelpCircle size={14} className="mr-1" />
                {t("help.apiSetup.howToGet")}
              </Button>
            </div>
            <LlmModeSelect value={form.mode} onChange={(next) => setMode(next as LLMMode)} disabled={saving} />
            ...
            <div className="flex items-center gap-2 pt-1">
              ...
              {llmConnection.status === "success" && (
                <span className="flex items-center text-xs text-success">
                  <CheckCircle2 size={14} className="mr-1" /> {llmConnection.message}
                </span>
              )}
              {llmConnection.status === "error" && (
                <span className="flex items-start text-xs text-error">
                  <XCircle size={14} className="mr-1 mt-0.5 shrink-0" />{" "}
                  <span className="leading-tight">{llmConnection.message}</span>
                </span>
              )}
            </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="space-y-2 pl-2 border-l-2 border-accent/30">
                <p className="text-xs leading-relaxed text-warning">{t("settings.global.embeddingIndependentHint")}</p>
                ...
                <div className="flex items-center gap-2 pt-1">
                  <Button
                    ...
                    disabled={saving || embeddingConnection.status === "testing" || !form.embeddingModel.trim()}
                  >
                    {embeddingConnection.status === "testing" ? <Spinner size="sm" className="mr-1" /> : null}
                    {t("common.actions.testEmbeddingConnection")}
                  </Button>
                  {embeddingConnection.status === "success" && (
                    <span className="flex items-center text-xs text-success">
                      <CheckCircle2 size={14} className="mr-1" /> {embeddingConnection.message}
                    </span>
                  )}
                  {embeddingConnection.status === "error" && (
                    <span className="flex items-start text-xs text-error">
                      <XCircle size={14} className="mr-1 mt-0.5 shrink-0" />{" "}
                      <span className="leading-tight">{embeddingConnection.message}</span>
                    </span>
                  )}
                </div>
              </div>
          ...
          <div className="space-y-2 border-t border-rule pt-5">
            <div className="flex flex-col gap-1.5">
              ...
              <p className="text-xs text-text/50">
                {t("settings.global.reactExtractionDescription", {
                  defaultValue:
                    "确认章节后用 AI 自动给笔记找跨章因果、归入剧情线。会多花几秒；关闭则用更快的基础提取。",
                })}
              </p>
            </div>
          </div>
          ...
          <div className="sticky bottom-0 -mx-4 md:-mx-6 flex justify-end gap-3 border-t border-rule bg-surface px-4 md:px-6 py-3">
            <Button tone="neutral" fill="plain" onClick={requestClose} disabled={saving}>
