 ...
 */
export function BackfillMemoryModal({ auPath, isOpen, onClose }: { auPath: string; isOpen: boolean; onClose: () => void }) {
  ...
  const allChapters = useMemo(
    () => (scan ? Object.keys(scan.factCountByChapter).map(Number).sort((a, b) => a - b) : []),
    [scan],
  );
  ...
  return (
    <Modal isOpen={isOpen} onClose={handleRequestClose} title={t('backfillMemory.title')}>
      <div className="space-y-5">
        ...
        {phase === 'confirm' && scan && (() => {
          if (!scan.embeddingConfigured || !scan.llmConfigured) return <p className="text-sm text-text/70">{t('backfillMemory.needConfig')}</p>;
          if (scan.totalConfirmed === 0) return <p className="text-sm text-text/70">{t('backfillMemory.noChapters')}</p>;
          return (
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex gap-2">
                    <Button tone="neutral" fill="plain" size="sm" onClick={() => setSelectedFacts(new Set(allChapters))}>{t('backfillMemory.selectAll')}</Button>
                    <Button tone="neutral" fill="plain" size="sm" onClick={() => setSelectedFacts(new Set())}>{t('backfillMemory.selectNone')}</Button>
                  </div>
                </div>
                ...
                <div className="max-h-[40vh] space-y-1 overflow-y-auto rounded-lg border border-black/10 p-2 dark:border-white/10">
                  {allChapters.map((num) => {
                    ...
                    return (
                      <label key={num} className="flex cursor-pointer items-center gap-2 rounded px-2 py-1 text-sm hover:bg-black/5 dark:hover:bg-white/5">
                        <input type="checkbox" className="accent-accent" checked={selectedFacts.has(num)} onChange={() => toggleFact(num)} />
                        <span className="text-text/90">{t('backfillMemory.chapterRow', { num })}</span>
                        <span className="text-xs text-text/40">
                          {count > 0 ? t('backfillMemory.chapterFactCount', { count }) : t('backfillMemory.chapterNoFacts')}
                        </span>
                      </label>
                    );
                  })}
              ...
              <p className="text-xs text-text/50">
                {t('backfillMemory.costHint', { summaries: scan.chaptersMissingSummary.length, facts: selectedFacts.size })}
              </p>

              {inScopeCount === 0 && (
                <p className="text-sm text-text/70">{t('backfillMemory.nothingToDo')}</p>
              )}
            </div>
          );
        })()}
        ...
        {phase === 'running' && (
          <div className="space-y-2">
            <p className="text-sm text-text/80">{t('backfillMemory.running', { done: progress.done, total: progress.total })}</p>
        ...
        <div className="flex justify-end gap-2 border-t border-black/10 pt-4 dark:border-white/10">
          {phase === 'confirm' && ready ? (
            <>
              ...
              <Button tone="accent" fill="solid" onClick={handleStart} disabled={inScopeCount === 0}>{t('backfillMemory.start')}</Button>
            </>
          ) : phase === 'running' ? (
            <Button tone="neutral" fill="outline" onClick={handleStop}>{t('backfillMemory.stop')}</Button>
          ) : phase !== 'scanning' ? (
