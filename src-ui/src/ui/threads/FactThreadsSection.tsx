...
export const FactThreadsSection = ({ auPath, threadIds, threadRoles }: FactThreadsSectionProps) => {
  ...
  useEffect(() => {
    ...
    listThreads(auPath)
      .then(ts => { if (alive) setThreads(ts); })
  ...
  const memberships = useMemo(
    () => ids.map(id => ({ id, thread: byId.get(id), role: threadRoles?.[id] ?? '' }))
  ...
  return (
    <div className="flex flex-col gap-2 border-t border-black/10 pt-4 dark:border-white/10">
      <div className="flex items-center justify-between gap-2 rounded-sm bg-drawer px-3 py-2 text-inv-text" style={headerGoldLines}>
        ...
        <div className="flex flex-col gap-2">
          {memberships.map(({ id, thread, role }) => (
            <div key={id} className="group relative rounded-sm border border-rule bg-surface/60 py-2.5 pl-4 pr-3">
              <span aria-hidden className="pointer-events-none absolute left-0 top-2.5 bottom-2.5 w-[2px] rounded-r bg-gold opacity-65" />
