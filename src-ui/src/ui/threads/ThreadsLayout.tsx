                ...
                actions={[
                  {
                    key: "new-thread",
                    element: (
                      <Button tone="accent" fill="solid" size="sm" onClick={() => setEditing(emptyDraft())}>
                        {t("threads.newThread")}
                      </Button>
                    ),
                  },
                ...
                {STATUS_ORDER.filter((s) => (grouped[s]?.length ?? 0) > 0).map((status) => (
                  <section key={status} className="flex flex-col gap-3" aria-labelledby={`thread-group-${status}`}>
                    ...
                    <div className="rounded-sm bg-drawer px-4 py-2.5 text-inv-text" style={headerGoldLines}>
                      <span
                        id={`thread-group-${status}`}
                        className="font-mono text-[9px] font-medium uppercase tracking-[0.18em] text-gold-bright"
                      >
                      ...
                      {grouped[status].map((th) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            {th.state ? (
                              <p className="line-clamp-2 font-serif text-[13px] leading-snug text-ink-muted">
                                {th.state}
                              </p>
                            ) : (
          ...
          <div className="space-y-4">
            ...
            <div className="flex items-center justify-between gap-2 border-t border-black/10 pt-4 dark:border-white/10">
              ...
              <div className="flex items-center gap-2">
                <Button tone="neutral" fill="plain" onClick={() => setEditing(null)} disabled={saving || deleting}>
                ...
                <Button
                  ...
                  fill="solid"
                  onClick={handleSave}
                  disabled={!editing.title.trim() || saving || deleting}
                >
