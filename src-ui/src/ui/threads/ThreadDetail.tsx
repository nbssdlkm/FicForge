...
export const ThreadDetail = ({ auPath, thread, facts, onBack, onEdit, onChanged }: ThreadDetailProps) => {
  ...
  const available = useMemo(() => {
    ...
    return facts
      .filter(f => !(f.thread_ids ?? []).includes(thread.id))
      .filter(f => !q || (f.content_clean ?? '').toLowerCase().includes(q) || (f.characters ?? []).some(c => c.toLowerCase().includes(q)))
      .sort((a, b) => (a.chapter ?? 0) - (b.chapter ?? 0));
    ...
    <div className="flex h-full w-full flex-col bg-background">
      ...
      <div className="flex flex-1 flex-col overflow-hidden md:flex-row">
        ...
        <aside className="shrink-0 space-y-4 border-b border-rule bg-surface/40 px-5 py-5 md:w-72 md:overflow-y-auto md:border-b-0 md:border-r">
          {thread.description ? (
            <div>
              <div className="mb-1 font-mono text-[9px] uppercase tracking-[0.18em] text-ink-faint">{t('threads.field.description')}</div>
        ...
        <main className="flex-1 overflow-y-auto px-4 py-5 md:px-6">
          <div className="mx-auto flex max-w-2xl flex-col gap-3">
            <div className="flex items-center justify-between gap-3 rounded-sm bg-drawer px-4 py-2.5 text-inv-text" style={headerGoldLines}>
              <span className="font-mono text-[9px] font-medium uppercase tracking-[0.18em] text-gold-bright">{t('threads.detail.nodesTitle')}</span>
              <Button tone="neutral" fill="plain" size="sm" className="h-7 gap-1 text-inv-text/85 hover:bg-gold-bright/10 hover:text-inv-text" onClick={() => { setPickerFilter(''); setPickerOpen(true); }}>
                ...
                return (
                  <div key={f.id} className="group relative rounded-sm border border-rule bg-surface py-3 pl-5 pr-3">
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-1.5">
                      {tags.map(tag => (
                        <span key={tag} className="rounded-[1px] border border-gold/30 bg-gold/15 px-[5px] py-[2px] font-mono text-[8px] font-medium uppercase tracking-[0.14em] text-gold">{tag}</span>
                      ))}
                      ...
                      {editingRoleId === f.id ? (
                        ...
                        <button
                          type="button"
                          onClick={() => { setEditingRoleId(f.id); setRoleDraft(role); }}
