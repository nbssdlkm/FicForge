...
function StatusBadge({ message, t }: { message: SimpleWritingDraftMessage; t: ReturnType<typeof useTranslation>['t'] }) {
...
function WritingDraftCardImpl({
  message,
  isStreaming,
  onAccept,
  onRegenerate,
  onDiscard,
}: WritingDraftCardProps) {
  ...
  const showActions =
    message.status === 'streaming' || message.status === 'pending' || message.status === 'error';
      ...
      {showActions && (
        <div className="flex flex-wrap gap-2 border-t border-rule pt-3">
          {(message.status === 'pending' || message.status === 'error') && (
            ...
            >
              {message.status === 'error' ? <RotateCcw size={12} className="mr-1" /> : <Check size={12} className="mr-1" />}
              {message.status === 'error'
                ? t('simple.draftCard.retryAccept', { defaultValue: '重试接受为第 {{num}} 章', num: message.chapterNum })
