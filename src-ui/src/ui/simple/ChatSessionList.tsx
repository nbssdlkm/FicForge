...
export function ChatSessionList({ sessions, activeId, onSelect, onCreate, onRename, onDelete }: ChatSessionListProps) {
  ...
  return (
    <aside className="hidden w-52 shrink-0 flex-col border-r border-rule bg-surface md:flex">
                  ...
                  <>
                    <button type="button" onClick={() => onSelect(s.id)} className="block w-full text-left">
                      <span className="flex items-center gap-1.5">
                        <MessageSquare size={12} className={`shrink-0 ${isActive ? "text-accent" : "text-ink-faint"}`} />
                        <span className={`truncate text-[12.5px] ${isActive ? "font-semibold text-text" : "text-text/80"}`}>
                          {s.title}
                        </span>
                      </span>
