...
export function useFactEditor(auPath: string, currentChapter: number, onSaved: () => void) {
    ...
    try {
      ...
      const assembledKnownTo: FactInfo["known_to"] =
        knownToMode === "unset"
          ...
          : knownToMode === "all"
            ? "all"
            : knownToMode === "reader_only"
              ? "reader_only"
              : finalKnownToNames.length > 0
                ? finalKnownToNames
                : null;
      ...
      const nextKnownTo = saved
        ? (saved.known_to ?? null)
        : "known_to" in updatedFields
          ? assembledKnownTo
          : origKnownTo;
      const nextHiddenFrom = saved
        ? Array.isArray(saved.hidden_from)
          ? saved.hidden_from
          : []
        : "hidden_from" in updatedFields
          ? finalHiddenFrom
          : (origHiddenFrom as string[]);
      setEditingFact((prev) =>
  ...
  const removeKnownToNameAt = (index: number) => setKnownToNames((prev) => prev.filter((_, i) => i !== index));
  ...
  const removeHiddenFromNameAt = (index: number) => setHiddenFromNames((prev) => prev.filter((_, i) => i !== index));
