...
function setup(currentChapter = 5) {
  ...
  const hook = renderHook(({ auPath }: { auPath: string }) => useFactEditor(auPath, currentChapter, onSaved), {
...
describe("useFactEditor · handleAddFact", () => {
  ...
  it("进行中重入：adding=true 时再次调用 no-op（防双击重复落库）", async () => {
    ...
    act(() => {
      firstAdd = hook.result.current.handleAddFact();
    });
  ...
  it("AU 切换竞态：响应落地时已换 AU → 不关 modal、不 showError（旧页面结果整体丢弃）", async () => {
    ...
    act(() => {
      addPromise = hook.result.current.handleAddFact();
    });
...
describe("useFactEditor · 对抗审整改（MED-4 竞态 + 引擎回传反映）", () => {
  it("保存 A 在飞时切到 B：迟到的回写被丢弃，B 的状态不被污染", async () => {
    ...
    act(() => {
      savePromise = hook.result.current.handleSaveFact();
    });
    ...
    // A 的请求此刻才完成
    await act(async () => {
      gate.resolve(factWith({ id: "f_A", known_to: ["甲"], hidden_from: ["乙"] }));
      await savePromise;
    });
  ...
  it("引擎回传的矛盾化解结果反映进编辑 state（同名同现 → 瞒着方胜）", async () => {
    ...
    vi.mocked(editFact).mockResolvedValueOnce(factWith({ id: "f_C", known_to: null, hidden_from: ["王妃"] }) as never);
    await act(() => hook.result.current.handleSaveFact());
