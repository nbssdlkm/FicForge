...
export function useWorkspaceMilestones(auPath: string) {
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: 边沿触发——体内全是 setter（非依赖），仅应随 auPath 变化回到初始快照；biome 判 auPath 多余，删掉会导致切 AU 不再复位（残留上一篇里程碑数）
  useEffect(() => {
    setCurrentChapter(1);
    setFactsCount(0);
    setPinnedCount(0);
  ...
  // biome-ignore lint/correctness/useExhaustiveDependencies: refreshKey 是写路径重取触发键（refreshMilestones() 递增它以重拉里程碑数），体内不直接读它；biome 判其多余，删掉会导致 confirm/undo/接受等写完后里程碑不刷新
  useEffect(() => {
    if (!auPath) return;
