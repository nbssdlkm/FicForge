...
function buildDefaultContent(name: string, category: LoreCategory): string {
  return category === 'worldbuilding'
    ? buildDefaultWorldbuildingContent(name)
    : buildDefaultCharacterContent(name);
}
...
export function useAuLoreEditor(
  ...
  const openFile = useCallback(async (name: string, category: LoreCategory) => {
