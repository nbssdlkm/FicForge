  ...
  return (
    <>
      <Modal
        ...
        onClose={closeCreate}
        title={selectedCategory === "worldbuilding" ? t("auLore.createTitleWorldbuilding") : t("auLore.createTitle")}
      >
        <div className="space-y-4">
          <p className="text-sm text-text/70">
            {selectedCategory === "worldbuilding"
              ? t("auLore.createDescriptionWorldbuilding")
              : t("auLore.createDescription")}
          </p>
          <Input
            value={createName}
            ...
            placeholder={
              selectedCategory === "worldbuilding"
                ? t("auLore.createPlaceholderWorldbuilding")
                : t("auLore.createPlaceholder")
            }
            autoFocus
          />
          <div className="flex justify-end gap-2">
            <Button tone="neutral" fill="plain" onClick={closeCreate}>
              {t("common.actions.cancel")}
            </Button>
            <Button tone="accent" fill="solid" onClick={handleCreate} disabled={!createName.trim()}>
      ...
      <Modal isOpen={importModalOpen} onClose={isSaving ? () => {} : closeImport} title={t("auLore.importTitle")}>
        <div className="space-y-4">
          ...
          <div className="max-h-[50vh] space-y-2 overflow-y-auto rounded-lg border border-black/10 p-2 dark:border-white/10">
            {importLoading ? (
              ...
              importCandidates.map((file) => (
                <label
                  key={file.name}
                  className="flex min-h-[44px] items-center gap-3 rounded-md px-3 py-2 hover:bg-black/5 dark:hover:bg-white/5 cursor-pointer"
                >
                  <input
                    type="checkbox"
          ...
          <div className="flex justify-end gap-2">
            <Button tone="neutral" fill="plain" onClick={closeImport} disabled={isSaving}>
              {t("common.actions.cancel")}
            </Button>
            <Button
              tone="accent"
              ...
              disabled={selectedImports.length === 0 || isSaving}
            >
