...
export function EmptyState({ icon, title, description, actions = [], compact = false }: EmptyStateProps) {
  return (
    <div className={`flex flex-col items-center justify-center px-6 text-center ${compact ? "py-8" : "py-12"}`}>
      ...
      <h3 className="font-display text-[22px] font-semibold leading-tight text-accent">{title}</h3>
      <div className="mt-2 max-w-xl whitespace-pre-line font-serif text-sm leading-6 text-ink-muted">{description}</div>
      {actions.length > 0 && (
