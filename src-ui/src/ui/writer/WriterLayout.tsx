...
export const WriterLayout = ({
  ...
  const { mode, showSettingsTooltip, handleModeChange, closeSettingsTooltip } = useWriterModeController({
    isMobile,
    isSettingsModeBusy,
    showToast,
    t,
  ...
  const displayContent = confirmedEditor.isViewingHistory
    ? confirmedEditor.viewingHistoryContent || ""
    : displayState.fallbackDisplayContent;
  ...
  const headerProps = {
    ...
    isSettingsModeBusy,
    isGenerating: generation.isGenerating,
    isViewingHistory: confirmedEditor.isViewingHistory,
    viewingHistoryNum: confirmedEditor.viewingHistoryNum,
    currentChapter: displayState.displayedChapter,
    metaModel: displayState.metaModel,
    metaChars: displayState.metaChars,
    metaDuration: displayState.metaDuration,
    sessionTemp: sessionParams.sessionTemp,
    chaptersDirty: dirtyChapters,
    onOpenExport: chrome.openExport,
    onOpenDirty: () => {
      chrome.openDirty(dirtyChapterNum);
  ...
  const headingChapter =
    confirmedEditor.isViewingHistory && confirmedEditor.viewingHistoryNum
  ...
  const showDraftLabel =
    !confirmedEditor.isViewingHistory && displayState.currentDraft !== null && draftCtrl.drafts.length > 0;
  const draftLabel = showDraftLabel ? `Draft ${String(draftCtrl.activeDraftIndex + 1).padStart(2, "0")}` : undefined;
  ...
  const chapterContentAreaProps = {
    ...
    isGenerating: generation.isGenerating,
    isViewingHistory: confirmedEditor.isViewingHistory,
    viewingHistoryContent: confirmedEditor.viewingHistoryContent,
    viewingHistoryNum: confirmedEditor.viewingHistoryNum,
    editingConfirmed: confirmedEditor.editingConfirmed,
    editingContent: confirmedEditor.editingContent,
    editingOriginalContent: confirmedEditor.editingOriginalContent,
    savingEdit: confirmedEditor.savingEdit,
    onEditingContentChange: confirmedEditor.setEditingContent,
    onSaveEdit: confirmedEditor.saveEditingConfirmed,
    onCancelEdit: confirmedEditor.cancelEditingConfirmed,
    currentDraft: displayState.currentDraft,
    onDraftChange: draftCtrl.handleCurrentDraftChange,
    displayContent,
    generationErrorDisplay: generation.generationErrorDisplay,
    onDismissError: generation.dismissError,
    onNavigate,
    fontSize,
    lineHeight,
    displayedChapter: headingChapter,
    displayedChapterTitle: headingTitle,
    draftLabel,
  };
  const footerProps = {
    footerCollapsed: chrome.footerCollapsed,
    onToggleCollapsed: chrome.toggleFooterCollapsed,
    isGenerating: generation.isGenerating,
    writeActionsDisabled: displayState.writeActionsDisabled,
    isSettingsModeBusy,
    isDiscarding: chapterActions.isDiscarding,
    currentChapter: displayState.currentChapter,
    instructionText: instructionInput.instructionText,
    onInstructionTextChange: instructionInput.setInstructionText,
    instructionInputRef: instructionInput.instructionInputRef,
    onGenerate: (type: "instruction" | "continue") => {
      void generation.handleGenerateFromInput(type);
    },
    drafts: draftCtrl.drafts,
    activeDraftIndex: draftCtrl.activeDraftIndex,
    onSelectDraft: draftCtrl.selectDraft,
    currentDraft: displayState.currentDraft,
    hasPendingDrafts: displayState.hasPendingDrafts,
    currentDraftMeta: displayState.currentDraftMeta,
    onOpenFinalize: chrome.openFinalizeConfirm,
    onRegenerate: () => {
      void generation.handleRegenerate();
    },
    onOpenDiscard: chrome.openDiscardConfirm,
    onOpenUndo: chrome.openUndoConfirm,
    onNavigateFacts: () => onNavigate("facts"),
    onOpenMobileTools: chrome.openMobileTools,
  ...
  const sidePanelProps = {
    ...
    unresolvedFacts,
    focusSelection: focusController.focusSelection,
    onFocusToggle: focusController.handleFocusToggle,
    onClearFocus: focusController.handleClearFocus,
    onContinueLastFocus: focusController.handleContinueLastFocus,
    lastConfirmedFocus,
    budgetReport: draftCtrl.budgetReport,
    contextLayers: displayState.contextLayers,
    layerSum: displayState.layerSum,
    sessionModel: sessionParams.sessionModel,
  ...
  const modalProps = {
    isFinalizeConfirmOpen: chrome.isFinalizeConfirmOpen,
    onCloseFinalizeConfirm: chrome.closeFinalizeConfirm,
    currentChapter: displayState.currentChapter,
    chapterTitle: chrome.chapterTitle,
    onChapterTitleChange: chrome.setChapterTitle,
    previewText: displayState.previewText,
    onConfirmFinalize: () => void chapterActions.handleConfirm(),
    isFinalizing: chapterActions.isFinalizing,
    hasDraft: displayState.currentDraft !== null,
    isDiscardConfirmOpen: chrome.isDiscardConfirmOpen,
    onCloseDiscardConfirm: chrome.closeDiscardConfirm,
    draftsCount: draftCtrl.drafts.length,
    onDiscardDrafts: () => void chapterActions.handleDiscardDrafts(),
    isDiscarding: chapterActions.isDiscarding,
    ...
    skipFactsPrompt: factsExtraction.skipFactsPrompt,
    factsPromptTitle: t("drafts.finalizeSuccess", { chapter: finalizedChapter }),
    onOpenExtractReview: () => void factsExtraction.handleOpenExtractReview(chapterActions.lastConfirmedChapter),
    onFactsManualNavigate: () => {
      factsExtraction.dismissFactsPrompt();
      onNavigate("facts");
    },
    ...
    onCloseExtractReview: () => {
      factsExtraction.closeExtractReview();
      instructionInput.focusInstructionInput();
    },
    ...
    onSaveExtracted: () => void factsExtraction.handleSaveExtracted(chapterActions.lastConfirmedChapter),
    savingExtracted: factsExtraction.savingExtracted,
    isUndoConfirmOpen: chrome.isUndoConfirmOpen,
    onCloseUndoConfirm: chrome.closeUndoConfirm,
    undoChapterNum: displayState.currentChapter - 1,
    onConfirmUndo: chapterActions.handleUndoConfirmed,
    isUndoing: chapterActions.isUndoing,
  };
  ...
  return (
    <>
            ...
            actions={
              <>
                <Button
                  ...
                  fill="plain"
                  size="sm"
                  className="h-11 text-xs md:h-6"
                  onClick={() => chrome.openDirty(dirtyChapterNum)}
                >
                  {t("dirty.goResolve")}
                </Button>
                <Button
                  ...
                  fill="plain"
                  size="sm"
                  className="h-11 text-xs text-text/50 md:h-6"
                  onClick={chrome.dismissDirtyBanner}
                >
                  {t("dirty.dismissBanner")}
                </Button>
              </>
            }
          />
            ...
            <div className="w-full max-w-[720px] space-y-6 px-4 py-4 md:px-8 md:py-10">
              {confirmedEditor.isViewingHistory && (
                <InlineBanner
                  ...
                  message={
                    <>
                      {t("workspace.chapterItem", { num: confirmedEditor.viewingHistoryNum })} ·{" "}
                      {t("writer.viewingHistory")}
                    </>
                  }
                  actions={
                    <>
                      <Button
                        ...
                        fill="plain"
                        size="sm"
                        onClick={confirmedEditor.startEditingConfirmed}
                        disabled={confirmedEditor.editingConfirmed}
                      >
                        {t("writer.editChapter")}
                      </Button>
                      <Button tone="neutral" fill="plain" size="sm" onClick={confirmedEditor.clearHistoryView}>
              ...
              <InlineBanner
                ...
                actions={
                  <Button
                    ...
                    fill="plain"
                    size="sm"
                    className="h-7 px-2 text-info"
                    onClick={closeSettingsTooltip}
                  >
      ...
      <WriterToolPanels
        ...
        onToggleRightCollapsed={chrome.toggleRightCollapsed}
        mobileToolsOpen={chrome.mobileToolsOpen}
        onCloseMobileTools={chrome.closeMobileTools}
        onOpenUndo={chrome.openUndoConfirm}
        onOpenExport={chrome.openExport}
        currentChapter={displayState.currentChapter}
        writeActionsDisabled={displayState.writeActionsDisabled}
      ...
      <DirtyModal
        isOpen={chrome.isDirtyOpen}
        onClose={chrome.closeDirty}
        auPath={auPath}
        chapterNum={chrome.dirtyTargetChapter}
        onResolved={() => {
          chrome.closeDirty();
          void loadData();
        }}
