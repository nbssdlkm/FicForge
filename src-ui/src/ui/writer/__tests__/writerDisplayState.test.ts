...
describe("deriveWriterDisplayState · busy 与内容", () => {
  it.each([
    "isGenerating", "isFinalizing", "isDiscarding", "isUndoing", "isSettingsModeBusy",
  ...
  it("展示内容优先级：streamText > 当前草稿 > 已定稿正文", () => {
    ...
    expect(
      deriveWriterDisplayState(baseOptions({ drafts: [makeDraft()] })).fallbackDisplayContent,
    ).toBe("草稿正文");
    expect(
  ...
  it("settingsFandomPath：从 auPath 截到 fandom 段；无 /aus/ 时原样返回", () => {
    ...
    expect(
      deriveWriterDisplayState(baseOptions({ auPath: "/data/fandoms/F" })).settingsFandomPath,
    ).toBe("/data/fandoms/F");
  });
  ...
  it("currentDraftSummary：非生成期按草稿 label 取；生成中强制 null（避免旧摘要挂在新流上）", () => {
    ...
    expect(
      deriveWriterDisplayState({ ...withDraft, isGenerating: true }).currentDraftSummary,
    ).toBeNull();
  });
});
...
describe("deriveWriterDisplayState · meta 回退链", () => {
  ...
  it("currentDraftMeta：合法时间戳 + 模型拼「 · 」；非法时间戳只留模型；无 generatedWith 为空串", () => {
    const valid = deriveWriterDisplayState(baseOptions({
      drafts: [makeDraft({
    ...
    const badTimestamp = deriveWriterDisplayState(baseOptions({
      drafts: [makeDraft({
...
describe("deriveWriterDisplayState · 上下文分层条", () => {
  ...
  it("零 token 的 P 层剔除；pinned（system）恒在；占比按层和归一", () => {
    const d = deriveWriterDisplayState(baseOptions({
      budgetReport: {
        system_tokens: 500, p1_tokens: 0, p2_tokens: 300,
        p3_tokens: 0, p4_tokens: 200, p5_tokens: 0,
      },
    }));
  ...
  it("极小层占比钳到最小 1%（条形图上仍可见）", () => {
    const d = deriveWriterDisplayState(baseOptions({
      budgetReport: {
        system_tokens: 10000, p2_tokens: 1,
        p1_tokens: 0, p3_tokens: 0, p4_tokens: 0, p5_tokens: 0,
      },
    }));
