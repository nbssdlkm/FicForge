# FicForge · 粮坊 — Codex 项目指南

## 项目概况

FicForge 是面向同人写手的 AI 辅助续写工具。**架构迁移（Python 后端 → TypeScript 统一核心引擎，M0–M5）已完成**，引擎同时支撑桌面端（Tauri）和移动端（Capacitor/PWA）。

**当前状态（2026-07-13）**：简版收敛、对话式 × 记忆栈融合（写文/对话双 tab 恒并列的单一主力版）均已完成，`writing_mode` 开关已物理退役。**代码层零 open 技术债（TD-001…023 全闭环）、零待决裁决点，全部已 push origin/main**；仅剩 5 条真机/人工验证待办。**逐会话的权威状态与待办在 `PROGRESS.md`**（每个工作会话收尾更新），本文件只保留长期有效的架构与规范。
...
> **简版收敛（已完成并再演进）**：Phase 1/2 把简版 fork 的 agent-harness 能力收敛回主仓（`services/` 下 `agent_loop` / `agent_telemetry` / `tool_args_repair` / `tool_stream_buffer` / `simple_chat_dispatch` / `estimate_simple_tokens`；`domain/simple_chat.ts` + `simple_tools_zod.ts`）。随后「对话式 × 记忆栈融合」（2026-06-28~07-02）收敛为单一主力版：`writing_mode` 字段与 flag 体系已物理退役，`config/simple_features.ts` 现仅保留 `SIMPLE_AGENT_MAX_ITER` 常量。
...
| M7 | 架构简化（同步退役 + ops 降级 audit log） | **已完成**（已合 main 并 push）：同步 UI 隐藏 + engine 同步引擎删除、`ops_merge.ts` 外科式迁 `src-engine/ops/`（剥离多设备合并/冲突检测、保留 rebuild/lamport 投影核心）、死 UI/API/locale/文档清理 |
| M8 | Memory 三层架构（Fact / Chapter Summary / Thread） | **已完成**（2026-06，真机验过） |
| M9 | ReAct 基础设施（生成 + 选择性提取） | **已完成**（2026-06；后续经多轮审计加固） |
| M10 | Retrospective rewrite + Archive 冷热分层 | **已完成**（2026-06；`run_archival_sweep` 自动档有意不接 confirm，走用户确认 UI） |

> **注**：M6–M10 源自 PRD v5（架构简化 + Memory 重设计），其中 M7–M10 均已完成，M6 未启动（触发条件见 D-0043）。**PRD v5 及 D-00xx 决策记录 / devlog 已不在仓库内**（`docs/internal/` 现存 `plans/` + `audit/`）—— 如需查阅在 Obsidian `D:\MY LIFE\FicForge\` 或归档处。

## 活跃工作（当前分支）

**当前分支：`main`，与 origin/main 完全同步（2026-07-13 大会话收官，工作树干净）。**

### 代码层现状：全部闭环

- **TD-001…023 技术债全清零**；五轮九维盲审收官（87.2/B、零 HIGH，报告在 `docs/internal/audit/`）；G 批修复 41/41；长期债①-⑤全清（巨型组件状态下沉六块 / 引擎测试 mock 迁共享 helper / snake-camel 命名收尾 / devDep 大版本升级[vite 8 Rolldown + TS 7 + tailwind 4 + plugin-react 6] / 零测试 hook 补测 102 用例）。
- 验证基线：引擎 1579 + UI 642 测试全绿、双 tsc 0、双包 biome lint 0、i18n 1289 对称。
- 近一个月主线（全过程见 `PROGRESS.md` 里程碑节）：M8/M9/M10 完成 → 对话式 × 记忆栈融合（单一主力版）→ 三轮全量审计 + 五轮盲审修复 → M3 事实消费三批（角色知情边界 known_to/hidden_from 注入 + 人工可见可改）→ G6 架构收尾（confirm/undo 记忆编排下沉引擎、context_assembler 拆三、位置参数对象化）。

### 待办（仅剩真机/人工 5 条，代码无法覆盖）

1. **Tauri 壳收权冒烟**：CSP/connect-src 收窄后桌面端全流程 + 导出到任意路径；构建机上剩 Rust 侧清理（lib.rs + Cargo.toml 删 plugin-http/plugin-opener 注册）。
2. **真机日常写作流验证**：改配置立即生效、切 tab 生成存活、PWA 更新横幅、iOS 刘海 safe-area、离线冷启动、低端机流式帧率。
3. **真 key 端到端实跑剩项**：backfill 实跑（本机需走代理才连得上硅基流动 embedding）+ 自定义 chatPath 网关 + 纯 UI 点击层。
4. **Android Manifest / variables.gradle 入库**：~~文件在 Windows 构建机~~ **2026-09 本机已解除 gitignore 整目录忽略并 `git add` 暂存（随下次 commit 生效）。**
5. **新 UI 可视化眼验**（留真机旅程）：归档候选徽标、导入完成态引导。
...
> **2026-07 现状**：原 `docs/internal/{prd,decisions,devlog,milestone,governance,prompts}` 子目录**已不在仓库内**；现存 `plans/` + `audit/`（2026-07 起审计/盲审报告入库于此）。PRD（v2/v4/v5）、D-00xx 决策记录、devlog 如仍需查阅，在 Obsidian `D:\MY LIFE\FicForge\` 或已归档。**别再引用这些已失效路径。**
...
- `docs/internal/audit/` — 2026-07 三轮全量审计 + 五轮九维盲审报告（最新 `2026-07-12-blind-audit-round5.md`，87.2/B）
