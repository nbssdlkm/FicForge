...
- [ ] 巨型组件状态下沉（按 hook 铁律分批）：✅ AuSettingsLayout（2026-07-09，31 useState→0，4 hooks + 4 回归测试）；✅ MobileOnboarding + MobileFandomView（2026-07-10，19+17 useState→0，5 hooks + 9 回归测试，移动端两块清完）；本分支树上剩 AuLoreLayout(946行/26) / SettingsChatPanel(1026行/8，主要是行数债) / FandomLoreLayout(734行/22) / GlobalSettingsModal(450行/20)——如其余四件在并行会话已完成，以合并后清单为准
