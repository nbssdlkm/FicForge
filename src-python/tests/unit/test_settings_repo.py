...
@pytest.mark.asyncio
async def test_file_not_exists_creates_default(tmp_path):
...
@pytest.mark.asyncio
async def test_missing_fields_filled_with_defaults(tmp_path):
    """字段缺失 → 补默认值后正确读取。"""
    (tmp_path / "settings.yaml").write_text(
...
@pytest.mark.asyncio
async def test_save_updates_updated_at(tmp_path):
    """写入后 updated_at 已更新。"""
    repo = LocalFileSettingsRepository(tmp_path)
    settings = await repo.get()
    old_time = settings.updated_at
    await repo.save(settings)
    settings2 = await repo.get()
    assert settings2.updated_at >= old_time
    assert settings2.updated_at != ""


@pytest.mark.asyncio
async def test_api_key_env_override(tmp_path, monkeypatch):
    """API Key 环境变量覆盖。"""
    (tmp_path / "settings.yaml").write_text(
...
@pytest.mark.asyncio
async def test_embedding_key_fallback(tmp_path):
